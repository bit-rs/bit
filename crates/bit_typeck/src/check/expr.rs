...
impl<'pkg, 'cx> ModuleCx<'pkg, 'cx> {
            ...
            Some(module) => match self.package.root.module(*module).fields.get(&field_name) {
                ...
                Some(def) => match def {
                    ...
                    ModuleDef::Function(f) => {
                        match f.publicity {
                            // If constant is public, we resolved field
                            Publicity::Public => Res::Value(Typ::FnDef(
                                f.value,
    ...
    fn infer_anonymous_fn(
        ...
        params
            .iter()
            .for_each(|p| self.resolver.define_local(&location, &p.0, p.1.typ.clone()));
        ...
        // result
        Typ::FnRef(
            params.iter().map(|p| p.1.typ.clone()).collect(),
            Box::new(ret),
        )
    }
