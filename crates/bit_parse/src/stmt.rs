...
use bit_ast::ast::{BinOp, Expression, Stmt};
...
impl<'file> Parser<'file> {
    ...
    /// Assignment parsing
    fn assignment(&mut self, address: Span, variable: Expression) -> Stmt {
        match variable {
            ...
            _ => {
                ...
                match op.tk_type {
                    ...
                    TokenKind::AddAssign => {
                        self.compound_assignment(BinOp::Add, address, variable)
                    }
                    TokenKind::SubAssign => {
                        self.compound_assignment(BinOp::Sub, address, variable)
                    }
                    TokenKind::MulAssign => {
                        self.compound_assignment(BinOp::Mul, address, variable)
                    }
                    TokenKind::DivAssign => {
                        self.compound_assignment(BinOp::Div, address, variable)
                    }
                    TokenKind::AndAssign => {
                        self.compound_assignment(BinOp::And, address, variable)
                    }
                    TokenKind::OrAssign => {
                        self.compound_assignment(BinOp::Or, address, variable)
                    }
                    TokenKind::XorAssign => {
                        self.compound_assignment(BinOp::Xor, address, variable)
                    }
    ...
    fn statement_requires_semi(&self, stmt: &Stmt) -> bool {
        match stmt {
            Stmt::Loop { .. } => false,
            Stmt::For { .. } => false,
            Stmt::Expr(Expression::If { .. }) => false,
            _ => true,
        }
    }

    /// Identifier statement
    fn id_stmt(&mut self) -> Stmt {
